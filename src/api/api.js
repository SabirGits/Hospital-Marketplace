...
import { clinics as clinicData } from "../data/clinics";
import { normalizeHospital, normalizeHospitalList } from "./adapters";
...
/* ============================================================
   DOCTORS / CLINICS — no backend for these yet, still mocked.
   Swap these for real apiClient calls once you add doctorRoutes /
   clinicRoutes + their controllers and models.
   ============================================================ */
...
export async function getDoctorById(id) {
  await delay();
  return doctorData.find((d) => d.id === id) || null;
}

export async function getClinics(params = {}) {
  await delay();
  let results = [...clinicData];
  if (params.city) results = results.filter((c) => c.city === params.city);
  return results;
}
