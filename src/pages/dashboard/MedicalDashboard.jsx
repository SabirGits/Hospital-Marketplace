...
export default function MedicalDashboard() {
  ...
  const [profile, setProfile] = useState({ businessName: "MediCare Diagnostics", medicalType: "Diagnostics", location: "Jaipur", description: "Full-service diagnostic lab offering pathology and imaging." });
  ...
  return (
    <DashboardLayout title={NAV.find((n) => n.key === tab)?.label} roleLabel="Medical" items={NAV} activeKey={tab} onSelect={setTab}>
      {tab === "profile" && (
        <form className="card" style={{ padding: 24, maxWidth: 640 }} onSubmit={(e) => { e.preventDefault(); showToast("Business profile updated"); }}>
