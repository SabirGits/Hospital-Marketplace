...
import { LayoutDashboard, UserCircle2, Stethoscope, ListChecks, MessageSquare, BarChart3, Settings, Eye, Phone, Globe, Plus, Trash2, Star, Clock, Pencil } from "lucide-react";
...
import {
  ...
  getDoctorsByHospital, createDoctor, updateDoctor, deleteDoctor,
} from "../../api/api";
...
export default function HospitalDashboard() {
  ...
  const [profile, setProfile] = useState({ name: hospital.name, description: hospital.description, website: hospital.website, phone: hospital.phone, address: hospital.address, image: "", facilities: hospital.facilities.join(", "), departments: hospital.departments.join(", ") });
  ...
  const loadDoctors = (hospitalId) => {
    if (!hospitalId) return;
    getDoctorsByHospital(hospitalId).then(setDoctorList).catch(() => {});
  };

  // Loads the real record for whoever's logged in — falls back to the demo
  // profile above if that fails (e.g. backend not running), so the dashboard
  // still works as a preview.
  useEffect(() => {
    getMyProfile()
      .then((real) => {
        setMyId(real._id);
        setProfile((p) => ({
          ...p,
          name: real.hospitalName || real.name || p.name,
          description: real.description || p.description,
          website: real.website || p.website,
          phone: real.phone || p.phone,
          address: real.address || p.address,
          image: real.image || "",
        }));
        loadDoctors(real._id);
      })
      .catch(() => {});
  }, []);
  ...
  const addDoctor = async (e) => {
    e.preventDefault();
    try {
      await createDoctor({ name: newDoctor.name, specialty: newDoctor.specialty });
      setNewDoctor({ name: "", specialty: "" });
      setAddOpen(false);
      loadDoctors(myId);
      showToast("Doctor added — now visible on the public site.");
    } catch (err) {
      showToast(err.message || "Couldn't add doctor");
    }
  };
  ...
  const removeDoctor = async (id) => {
    try {
      ...
      showToast(err.message || "Couldn't remove doctor");
    }
  };
  ...
  const saveSchedule = async (e) => {
    e.preventDefault();
    try {
      await updateDoctor(scheduleDoctor.id, { schedule: scheduleDraft });
      loadDoctors(myId);
      showToast(`${scheduleDoctor.name}'s schedule updated`);
      setScheduleDoctor(null);
    } catch (err) {
      showToast(err.message || "Couldn't update schedule");
    }
  };
    ...
    <DashboardLayout title={NAV.find((n) => n.key === tab)?.label} roleLabel="Hospital" items={NAV} activeKey={tab} onSelect={setTab}>
      ...
      {tab === "profile" && (
        <form className="card" style={{ padding: 24, maxWidth: 640 }} onSubmit={saveProfile}>
          <ImageUploadField label="Hospital Cover Photo" value={profile.image} onChange={(v) => setProfile({ ...profile, image: v })} />
