...
export default function ClinicDashboard() {
  ...
  const [profile, setProfile] = useState({ name: clinic.name, specialty: clinic.specialty, location: clinic.location, hours: clinic.hours, services: clinic.services.join(", "), image: "" });
  ...
  useEffect(() => {
    getMyProfile()
      .then((real) => {
        setProfile((p) => ({
          ...p,
          name: real.clinicName || real.name || p.name,
  ...
  const saveProfile = async (e) => {
    e.preventDefault();
    try {
      await updateMyProfile({ clinicName: profile.name, address: profile.location, image: profile.image });
    ...
    <DashboardLayout title={NAV.find((n) => n.key === tab)?.label} roleLabel="Clinic" items={NAV} activeKey={tab} onSelect={setTab}>
      {tab === "profile" && (
        <form className="card" style={{ padding: 24, maxWidth: 640 }} onSubmit={saveProfile}>
          <ImageUploadField label="Clinic Cover Photo" value={profile.image} onChange={(v) => setProfile({ ...profile, image: v })} />
