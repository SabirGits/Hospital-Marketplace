...
import { getHospitalById as fetchHospitalById, rateHospital, getDoctorsByHospital } from "../api/api";
...
export default function HospitalDetails() {
  ...
  useEffect(() => {
    if (!hospital) return;
    const demoMatches = demoDoctors.filter((d) => d.hospitalId === hospital.id);
    getDoctorsByHospital(hospital.id)
      .then((real) => setRelatedDoctors([...real, ...demoMatches].slice(0, 6)))
      .catch(() => setRelatedDoctors(demoMatches.slice(0, 6)));
  }, [hospital]);
        ...
        {tab === "Reviews" && (
          <div>
            <div className="review-summary">
              ...
              <div>
                <p className="text-muted mb-8" style={{ fontSize: "var(--fs-sm)" }}>Rate this hospital</p>
                <RatingInput onRate={async (value) => {
                  const updated = await rateHospital(hospital.id, value);
                  setHospital((h) => ({ ...h, rating: updated.rating, reviews: updated.reviews }));
                }} />
              </div>
            </div>
